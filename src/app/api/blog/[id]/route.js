...
export async function PUT(req, ctx){
    ...
    try {
        ...
        if(blog?.authorId?._id.toString() !== decodedToken._id.toString()){
            return new Response(JSON.stringify({msg: "Only Author can update the blog"}), {status: 403})    
        }
...
export async function DELETE(req, ctx){
    ...
    try {
        ...
        if(blog?.authorId?._id.toString() !== decodedToken._id.toString()){
            return new Response(JSON.stringify({msg: "Only Author can delete the blog"}), {status: 403})    
        }
        
        await Blog.findByIdAndDelete(id);

        return new Response({msg: "blog deleted successfully"}, {status: 200})
    } catch (error) {
        return new Response(JSON.stringify(null), {status: 500})
    }
}
